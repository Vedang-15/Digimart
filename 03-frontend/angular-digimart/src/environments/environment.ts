...
Each of this environment signify a different server, a different port. Now whil our backend is runnign on thiese differnt servers/environments, we cant every time go an manually change the urls in servic class that call the backend server api(each environment will hav a different servr, hence different port and different url). Thus we make use of environment files providwd by angular.
