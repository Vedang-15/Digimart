...
public class MyDataRestConfig implements RepositoryRestConfigurer {
    ...
    public void configureRepositoryRestConfiguration(RepositoryRestConfiguration config, CorsRegistry cors) {
        ...
        // below code is written so that we are able to send/expose ids of productCategory class. Spring data REST, when it sends JSON data to browser(when browser visits http://localhost/api/product-category), does not send id, it sends rest of the info. Ids are not sent.
        // This is default property of spring data rest. Similar is the case with product entity class. When browser visits http://localhost:8080/api/products, all products are snt in JSON format but the sent JSON data does not have id field. But in case of Product-category class,
